...
versions=$(curl -s https://dl.suckless.org/tools/ | grep -Eo 'href="[^\"]+"' | cut -d'"' -f2 | grep '^slstatus-.*\.tar\.gz$' | sort -r)
...
for i in $(seq 0 $((${#versions_array[@]}-1)) | tac); do
  ...
  selected_url="https://dl.suckless.org/tools/$selected_version"
